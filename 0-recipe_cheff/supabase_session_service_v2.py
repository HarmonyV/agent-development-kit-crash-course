...
def _merge_state(app_state: dict, user_state: dict, session_state: dict):
...
class SupabaseSessionService(BaseSessionService):
    ...
    def _get_app_state(self, app_name: str) -> dict[str, Any]:
        response: APIResponse = self.supabase.table("app_states").select("state").eq("app_name", app_name).maybe_single().execute()
        return response if response else {}

    def _get_user_state(self, app_name: str, user_id: str) -> dict[str, Any]:
        response: APIResponse = self.supabase.table("user_states").select("state").eq("app_name", app_name).eq("user_id", user_id).maybe_single().execute()
        return response if response else {}
    ...
    def create_session(
        ...
        *,
        ...
        if not self.supabase.table("app_states").select("app_name").eq("app_name", app_name).maybe_single().execute():
            self.supabase.table("app_states").insert({"app_name": app_name, "state": {}}).execute()
        if not self.supabase.table("user_states").select("app_name").eq("app_name", app_name).eq("user_id", user_id).maybe_single().execute():
        ...
        merged_state = _merge_state(app_state, user_state, created_session_data["state"])
    ...
    def get_session(
        ...
        *,
        ...
        app_state = self._get_app_state(app_name)
        user_state = self._get_user_state(app_name, user_id)
        session_state_data = db_session["state"]
